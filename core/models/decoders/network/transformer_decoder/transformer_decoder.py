...
from ....backbones.vit import get_2d_sincos_pos_embed
import math
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.bbox_embed = MLP(hidden_dim, hidden_dim, 4, 3) if (peddet_cfgs is not None or share_bbox_branch) else None
    ...
    def forward_reid_prediction_heads(self, output, norm):
        if self.reid_cfgs.head_type == 'cat':
            # decoder_output = self.decoder_norm(output)
    ...
    def forward_peddet(self, x, mask_features):
        ...
        for i in range(self.num_layers):
            ...
            output = self.transformer_cross_attention_layers[i](
                ...
                memory_mask=None,
                memory_key_padding_mask=None,  # here we do not apply masking on padded region
                pos=pos[level_index] if self.cross_pos_embed == "sincos" else self.get_vis_token_pos_embed(size_list[level_index], batch=bs),
